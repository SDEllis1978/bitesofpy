...
class BridgeHand:
    def __init__(self, cards: Sequence[Card]):
        ...
        for suit in self.suits.values():
            suit.sort(key=lambda x: x.value)
    ...
    def __str__(self) -> str:
        ...
        """
        out = ""
        for suit in sorted(self.suits, key=lambda x: x.value):
            out += f"{suit.name}:"
    ...
    def ltc(self) -> int:
        ...
        for cards in self.suits.values():
            top_3 = cards[:3]
            honours = 'AKQ'[:len(top_3)]
            losers += sum(card.name not in honours for card in top_3)
        return losers
